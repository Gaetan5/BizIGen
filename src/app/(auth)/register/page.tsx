...
import { Mail, Lock, User, Loader2, ArrowLeft, Rocket, Check, Star, Shield, Crown } from 'lucide-react';
...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.ok) {
        toast.success('Bienvenue sur BizGen AI !');
        window.location.href = '/onboarding';
      } else if (result?.error) {
        toast.error('Connexion automatique échouée. Veuillez vous connecter manuellement.');
        router.push('/login');
      } else {
        router.push('/login');
      }
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-primary via-[hsl(35,85%,45%)] to-[hsl(40,80%,40%)]">
        ...
        <div className="relative z-10 flex flex-col justify-center p-14 text-white">
          ...
          >
            Lancez votre projet en <span className="text-white underline decoration-wavy decoration-white/50">toute confiance</span>
          </motion.h1>
          ...
          >
            Créez votre compte gratuit et accédez à tous les outils pour structurer votre business et convaincre vos partenaires.
          </motion.p>
          ...
          >
            <FeatureItem icon={<Check className="w-6 h-6" />} text="Plan gratuit sans carte bancaire" delay={0} />
            <FeatureItem icon={<Star className="w-6 h-6" />} text="3 projets inclus pour démarrer" delay={0.1} />
            <FeatureItem icon={<Shield className="w-6 h-6" />} text="Support client réactif" delay={0.2} />
          </motion.div>
        </div>
      </div>

      {/* Right Side - Register Form */}
      <div className="flex-1 flex items-center justify-center p-8 bg-gradient-to-br from-background via-background to-muted/20 overflow-y-auto">
        <motion.div 
